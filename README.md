...
- **🤝 Whip Room** — the whip operation for any bill: all 51 offices' Legislative Directors from your imported staff roster (replaceable in-app via CSV), an editable status board that persists per bill (sponsors auto-marked signed-on), what-we-have math against **26 (majority)** and **34 (veto-proof)**, **predicted positive/negative response chances** (status + sign-on model), **alliance blocs** computed from who actually co-sponsors together (plus the real political factions), and a **message blast** that personalizes a template per office and opens prefilled drafts (bulk BCC or per-office) — the app prepares, a human sends. Every board save logs **movement history** (who moved, when, which way, and the chance the model gave at that moment), and a **calibration readout** shows how the model's predictions compare with what actually happened.
...
- **🔮 Sign-on Predictor** — predicts co-sponsorship: which bills a member is most/least likely to sign, and for a bill, which members to whip. It's a **transparent multi-factor model** — sponsorship topic-focus, coalition patterns, and momentum are always on, and you can blend in **previous voting behavior** (roll-calls), **district concerns (311 constituent demand)**, and **public statements** (web-sourced). Every prediction shows each factor's 0–1 contribution so you see *why*, with a labeled caveat. Directly supports decisions to push, hold, redraft, or amend.
